...
def bytes2AnonArray(bytes_data, byte_type = "uint8_t"):
    ...
    #print("bytes2AnonArray: {}".format(binascii.hexlify(bytes_data)))
    #print("ebug exit 2")
    #sys.exit(1)
...
class Gen:
    ...
    def run(self):
        ...
        if args.device_hi:
            self.device_keep = -1
            self.load_pcap(args, self.loop_cb_devmax)
...
''')
        ...
        self.load_pcap(args, self.loop_cb)
...
''')
    ...
    def load_pcap(self, args, loop_fn):
        if sys.version_info[0] == 2 and args.pylibpcap:
            p = pcap.pcapObject()
            p.open_offline(args.fin)
            p.loop(-1, self.loop_cb)
            return

        with open(args.fin, 'rb') as fp:
            scanner = pcapng.FileScanner(fp)
            for block in scanner:
                if not isinstance(block, pcapng.blocks.EnhancedPacket):
                    continue
                loop_fn(block.captured_len, block.packet_data, block.timestamp)
...
if __name__ == "__main__":
    ...
    # pylibpcap only works on Python 2, but it's 5 times faster and known to work well.
    # pcapng is actively maintained, but is considerably slower. It's the only option for Python 3.
    if sys.version_info[0] == 2:
        add_bool_arg(parser, '--pylibpcap', default=True, help='Use pylibpcap instead of slower pcapng library.')
